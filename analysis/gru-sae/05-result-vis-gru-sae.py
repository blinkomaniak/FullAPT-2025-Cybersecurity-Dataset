"""
Stage 05: GRU-SAE Result Visualization with Temporal Analysis

This script creates comprehensive visualizations for GRU-SAE anomaly detection results.
...
# === CLI Arguments ===
parser = argparse.ArgumentParser(description="Visualize GRU-SAE anomaly detection results")
...
# === Extract GRU encoder for latent space analysis ===
print("🧠 Extracting GRU encoder for latent space analysis...")

def extract_gru_encoder(model):
    """Extract encoder part from GRU autoencoder"""
...
encoder = extract_gru_encoder(model)
...
axes[0, 0].set_title('GRU Encoder Latent Space (PCA)')
...
axes[0, 1].scatter(Z_tsne[attack_mask, 0], Z_tsne[attack_mask, 1], 
                  s=20, alpha=0.6, label=f'Attack (n={np.sum(attack_mask)})', color='red')
axes[0, 1].set_title('GRU Encoder Latent Space (t-SNE)')
...
axes[0].set_title('GRU-SAE Performance Metrics')
...
arch_text = f"""GRU-SAE Architecture:
...
print("✅ GRU-SAE visualization complete! Artifacts saved:")
...
print("🏁 GRU-SAE visualization complete.")