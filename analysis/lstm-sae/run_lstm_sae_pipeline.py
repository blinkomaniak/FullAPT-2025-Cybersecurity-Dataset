#!/usr/bin/env python3
"""
LSTM-SAE Pipeline Runner

Automatically executes all 5 stages of the LSTM-SAE pipeline:
1. Data Aggregation
2. Sequence Encoding  
3. Model Training
4. Model Evaluation
...
Usage:
    python run_lstm_sae_pipeline.py --dataset bigbase --version v1 --config-preset basic
    python run_lstm_sae_pipeline.py --dataset unraveled --version v1 --config-preset network --subdir network-flows
"""
...
def get_config_paths(dataset, config_preset):
    ...
    if dataset == "bigbase":
        if config_preset == "basic":
            schema_config = f"{config_dir}/schema-bigbase-lstm-sae-v1.json"
            encoding_config = f"{config_dir}/encoding-bigbase-lstm-sae-v1.json"
        elif config_preset == "extended":
            schema_config = f"{config_dir}/schema-bigbase-lstm-sae-v2.json"
            encoding_config = f"{config_dir}/encoding-bigbase-lstm-sae-v2.json"
        else:
            raise ValueError(f"Unknown config preset for bigbase: {config_preset}")
    
    elif dataset == "unraveled":
        if config_preset == "network":
            schema_config = f"{config_dir}/schema-unraveled-lstm-sae-v1.json"
            encoding_config = f"{config_dir}/encoding-unraveled-lstm-sae-v1.json"
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Run complete LSTM-SAE pipeline")
    ...
    parser.add_argument("--model", default="lstm-sae", help="Model name")
    ...
    print(f"{Colors.BOLD}🔧 LSTM-SAE Pipeline Configuration{Colors.ENDC}")
