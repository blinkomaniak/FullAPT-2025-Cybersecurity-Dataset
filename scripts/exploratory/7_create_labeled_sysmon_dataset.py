...
class LabeledSysmonDatasetCreator:
    ...
    def __init__(self, apt_type: str, run_id: str):
        ...
        self.tactic_colors = {
            'initial-access': '#DC143C',      # Crimson Red
    ...
    def create_labeled_dataset(self, sysmon_df: pd.DataFrame, traced_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Create mapping key function
        def create_mapping_key(row):
            return f"{row['EventID']}_{row['Computer']}_{row['timestamp']}"
        
        # Create mapping keys for sysmon dataset (optimized)
        self.logger.info("🗝️ Creating mapping keys for sysmon dataset")
        sysmon_df['mapping_key'] = (sysmon_df['EventID'].astype(str) + '_' + 
                                   sysmon_df['Computer'].astype(str) + '_' + 
                                   sysmon_df['timestamp'].astype(str))
        ...
        for _, traced_row in traced_df.iterrows():
            ...
            try:
                ...
                master_row = master_df[master_df['OriginalRowNumber'] == seed_row_number]
                if len(master_row) == 0:
    ...
    def create_tactics_timeline_plot_v2(self, labeled_df: pd.DataFrame) -> bool:
        ...
        self.logger.info("📈 Creating tactics timeline plot (v2 - shared utilities)")
        
        # Use shared plotting utility
        return plot_tactics_timeline_v2(labeled_df, self.results_dir, self.logger)
    ...
    def run(self):
        ...
        try:
            ...
            if SHARED_UTILS_AVAILABLE and FeatureFlags.USE_SHARED_PLOTTING:
                ...
                self.create_tactics_timeline_plot_v2(labeled_df)  # Pass full dataset for gray background
            else:
