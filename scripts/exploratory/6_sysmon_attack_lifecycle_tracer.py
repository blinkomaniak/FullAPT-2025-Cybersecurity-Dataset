...
- Local/Remote/Tactic selection support from all_target_events_run-X.csv
...
class MultiEventIDAttackLifecycleTracer:
    ...
    def __init__(self, debug: bool = False):
        ...
        self.stats = {
            ...
            'local_originators_selected': 0,
            'remote_originators_selected': 0,
    ...
    def load_selected_originators(self, originators_file: Path) -> bool:
        ...
        Supports Local/Remote/Tactic/Technique column structure.
        ...
        try:
            ...
            has_local_remote = 'Local' in target_events_df.columns and 'Remote' in target_events_df.columns
            has_tactic = 'Tactic' in target_events_df.columns
            has_technique = 'Technique' in target_events_df.columns
            
            if not has_local_remote:
                self.logger.error("❌ CSV file must have Local and Remote columns")
                return False
            
            required_columns = ['Local', 'Remote', 'OriginalRowNumber']
            ...
            # Priority: Tactic > Local > Remote (to preserve Tactic/Technique information)
            
            # Start with all selected entries (Local or Remote marked as 'X')
            local_or_remote_selected = target_events_df[
                (target_events_df['Local'].astype(str).str.strip().str.upper() == 'X') |
                (target_events_df['Remote'].astype(str).str.strip().str.upper() == 'X')
            ].copy()
            
            if len(local_or_remote_selected) == 0:
                self.logger.warning("⚠️ No attack originators selected in Local or Remote columns")
                return False
            
            # Add processing type based on which column was marked
            local_or_remote_selected['processing_type'] = local_or_remote_selected.apply(
                lambda row: 'Remote' if str(row['Remote']).strip().upper() == 'X' else 'Local', axis=1
            )
            
            # Override processing type to 'Tactic' if Tactic information is available
            if has_tactic:
                tactic_mask = (
                    (local_or_remote_selected['Tactic'].astype(str).str.strip() != '') &
                    (local_or_remote_selected['Tactic'].astype(str).str.strip().str.upper() != 'NAN')
                )
                local_or_remote_selected.loc[tactic_mask, 'processing_type'] = 'Tactic'
            
            # Use only unique OriginalRowNumber entries (no duplicates)
            selected_df = local_or_remote_selected.drop_duplicates(subset=['OriginalRowNumber'], keep='first')
            
            if len(selected_df) == 0:
                self.logger.warning("⚠️ No attack originators selected in Local, Remote, or Tactic columns")
                return False
            
            # Statistics by EventID type (avoid duplication from Local+Tactic)
            # Get unique rows by dropping duplicates based on OriginalRowNumber
            unique_selected_df = selected_df.drop_duplicates(subset=['OriginalRowNumber'], keep='first')
            ...
            self.stats['local_originators_selected'] = processing_type_counts.get('Local', 0)
            self.stats['remote_originators_selected'] = processing_type_counts.get('Remote', 0)
            ...
            self.logger.info(f"   🏠 Local originators: {self.stats['local_originators_selected']}")
            self.logger.info(f"   🌐 Remote originators: {self.stats['remote_originators_selected']}")
            ...
            for i, orig in enumerate(self.selected_originators, 1):
                processing_type = orig.get('processing_type', 'Local')
                row_num = orig['OriginalRowNumber']
                ...
                type_emoji = '🏠' if processing_type == 'Local' else '🌐' if processing_type == 'Remote' else '🏷️'
    ...
    def get_originator_event_from_sysmon(self, originator_info: Dict) -> Optional[pd.Series]:
        ...
        try:
            original_row_number = originator_info['OriginalRowNumber']
    ...
    def trace_eventid1_lifecycle(self, originator_event: pd.Series, originator_info: Dict) -> Dict[str, any]:
        ...
        if process_pid is None:
            self.logger.warning(f"⚠️ No valid ProcessId for EventID 1 originator row {originator_info['OriginalRowNumber']}")
        ...
        traced_events = {
            ...
            'analysis_details': {
                ...
                'computer': computer,
                'originator_row': originator_info['OriginalRowNumber'],
    ...
    def _lookup_sysmon_row_by_original_row_number(self, original_row_number: int) -> pd.Series:
        """Look up the actual Sysmon row from CSV using the original row number."""
        try:
            # The original row number in all_target_events_run-04.csv corresponds to the actual line in sysmon CSV (1-indexed, including header)
            # Since pandas is 0-indexed and excludes the header row, we need to subtract 2
            actual_row_index = original_row_number - 2  # -1 for 0-indexing, -1 for header row
            
            if 0 <= actual_row_index < len(self.sysmon_df):
                return self.sysmon_df.iloc[actual_row_index]
            else:
                self.logger.warning(f"⚠️ Row number {original_row_number} is out of bounds (CSV has {len(self.sysmon_df)} rows)")
                return None
                
        except Exception as e:
            self.logger.error(f"❌ Error looking up Sysmon row {original_row_number}: {e}")
            return None
    ...
    def create_file_event_timeline(self, originator_event: pd.Series, originator_info: Dict) -> Dict[str, any]:
        ...
        try:
            ...
            # Look up the actual Sysmon row from CSV using OriginalRowNumber
            actual_sysmon_row = self._lookup_sysmon_row_by_original_row_number(originator_info['OriginalRowNumber'])
            ...
            if actual_sysmon_row is not None:
                ...
                self.logger.warning(f"⚠️ Could not find actual Sysmon row for OriginalRowNumber {originator_info['OriginalRowNumber']}")
    ...
    def process_all_originators(self) -> bool:
        ...
        for originator_info in self.selected_originators:
            try:
                originator_row = originator_info['OriginalRowNumber']
                event_id = originator_info['EventID']
                processing_type = originator_info.get('processing_type', 'Local')
                ...
                # Only increment if this is a new unique originator row (avoid duplicates from Local+Tactic)
    ...
    def _create_timeline_plot(self, traced_data: Dict, output_dir: Path) -> Optional[Path]:
        ...
        try:
            ...
            if 'analysis_details' in traced_data:
                ...
                events_to_plot = []
                for event in traced_data['all_events']:
    ...
    def _create_labeled_dataset_OLD_UNUSED(self, traced_events_file: Path, output_file: Path, apt_run_dir: Path) -> bool:
        ...
        try:
            ...
            def create_mapping_key(row):
                """Create a unique key for matching events."""
                return f"{row['EventID']}_{row['Computer']}_{row['timestamp']}"
    ...
    def create_tactics_timeline_plot(self, output_dir: Path) -> Optional[Path]:
        ...
        # Convert Unix epoch milliseconds to datetime with error handling
        try:
            all_sysmon_df['datetime'] = pd.to_datetime(all_sysmon_df['timestamp'], unit='ms', errors='coerce')
            # Remove any invalid timestamps
            invalid_timestamps = all_sysmon_df['datetime'].isna().sum()
        ...
        # Create mapping dictionary from traced events for tactic lookup
        tactic_map = {}
        for _, row in traced_df.iterrows():
            key = row['mapping_key']
            tactic_map[key] = row['Tactic']
        
        # Separate malicious from benign events using mapping
        malicious_mask = all_sysmon_df['mapping_key'].isin(tactic_map.keys())
        benign_events = all_sysmon_df[~malicious_mask]
        malicious_events = all_sysmon_df[malicious_mask].copy()
        
        # Add tactic information to malicious events
        malicious_events['Tactic'] = malicious_events['mapping_key'].map(tactic_map)
        
        # Plot benign events as pale gray background
        ...
        # Now organize malicious events by tactic
        tactics_events = {}
        ...
        for tactic in sorted(tactics_events.keys()):
            ...
            # Create scatter plot for this tactic with higher zorder to appear on top
        ...
        if len(malicious_events) > 0:
            all_eventids.update(malicious_events['EventID'].unique())
    ...
    def export_traced_events_to_csv(self, output_dir: Path) -> Optional[Path]:
        ...
        events_before_dedup = len(all_events)
        all_events = self._deduplicate_events(all_events)
        events_after_dedup = len(all_events)
        
        # Update statistics to reflect post-deduplication counts
        self.stats['total_traced_events'] = events_after_dedup
        self.stats['events_removed_by_deduplication'] = events_before_dedup - events_after_dedup
        self.stats['deduplication_rate_percent'] = round((self.stats['events_removed_by_deduplication'] / events_before_dedup * 100), 1) if events_before_dedup > 0 else 0.0
        
        # Recalculate events_by_computer and events_by_eventid with deduplicated data
        ...
        events_df = events_df.sort_values('timestamp_dt')
        
        # Drop the helper columns
        events_df = events_df.drop(columns=['timestamp_dt', '_sysmon_row_index'], errors='ignore')
        
        # Add manual reassignment columns
        events_df['Correct_SeedRowNumber'] = ''  # Empty for manual filling
        events_df['Attribution_Notes'] = ''     # For manual notes
        
        # Reorder columns to put Tactic, Technique, OriginatorRow first
        priority_columns = ['Tactic', 'Technique', 'OriginatorRow', 'Correct_SeedRowNumber', 'Attribution_Notes']
        remaining_columns = [col for col in events_df.columns if col not in priority_columns]
    ...
    def save_analysis_results(self, output_dir: Path) -> bool:
        ...
        try:
            ...
            # Count unique originators (avoid Local+Tactic duplication)
                ...
                if 'analysis_details' in traced_data:
                    ...
                    results[str(row)] = {
                        'analysis_details': {
                            ...
                            'processing_type': traced_data.get('processing_type', 'Local'),
